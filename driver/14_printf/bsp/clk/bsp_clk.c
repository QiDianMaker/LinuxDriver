...
/*
 * @brief  初始化系统时钟，设置系统时钟为 528Mhz，并且设置 PLL2 和 PLL3 各个
...
void imx6u_clkinit(void)
{
	unsigned int reg = 0;
	/* 1、设置ARM内核时钟为528MHz */
	/* 1.1、判断当前ARM内核是使用的那个时钟源启动的，正常情况下ARM内核是由pll1_sw_clk驱动
	 * 的，而pll1_sw_clk有两个来源：pll1_main_clk和tep_clk(参考手册648页)。     
	 * 如果我们要让ARM内核跑到528M的话那必须选择pll1_main_clk作为pll1的时钟源。
	 * 		如果我们要修改pll1_main_clk时钟的话就必须先将pll1_sw_clk从pll1_main_clk 
	 * 切换到 step_clk,当修改完pll1_main_clk以后在将pll1_sw_clk切换回pll1_main_clk
	 * 而step_clk的时钟源可以选择板子上的24MHz晶振。		
	 */
	 ...
	 *      bit13: 1 使能时钟输出
	 *      bit[6:0]: 88, 由公式：
	 *      Fout = Fin * div_select / 2.0，1056=24*div_select/2.0,
	 *      得出：div_select = 88  
	 */
	/* 配置pll1_main_clk=1056MHz */
